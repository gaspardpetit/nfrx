	...
	for {
		...
		case "open":
			if r.token != "" && f.Auth != r.token {
				_ = r.send(ctx, mcpwire.Frame{T: "open.fail", SID: f.SID, Code: "MCP_UNAUTHORIZED", Msg: "unauthorized"})
				continue
			}
			_ = r.send(ctx, mcpwire.Frame{T: "open.ok", SID: f.SID})
		case "rpc":
		...
		case "close":
			...
			r.mu.Unlock()
		case "ping":
			_ = r.send(ctx, mcpwire.Frame{T: "pong"})
		case "pong":
			// ignore
		}
	}
...
func (r *RelayClient) handleRPC(ctx context.Context, f mcpwire.Frame) {
	rpcCtx := ctx
	...
	_ = r.send(ctx, mcpwire.Frame{T: "rpc", SID: f.SID, Payload: resp})
	_ = r.send(ctx, mcpwire.Frame{T: "close", SID: f.SID, Msg: "done"})
}
...
func (r *RelayClient) pingLoop(ctx context.Context) {
	...
	for {
		select {
		case <-ticker.C:
			_ = r.send(ctx, mcpwire.Frame{T: "ping"})
		case <-ctx.Done():
			return
		}
	}
...
func (r *RelayClient) send(ctx context.Context, f mcpwire.Frame) error {
