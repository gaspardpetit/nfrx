...
func (m *MetricsRegistry) UpdateWorker(id string, maxConcurrency, embeddingBatchSize int, models []string) {
    m.mu.Lock(); if w, ok := m.workers[id]; ok { w.maxConcurrency = maxConcurrency; w.preferredBatchSize = embeddingBatchSize; if models != nil { w.labels = models } } ; m.mu.Unlock()
}
