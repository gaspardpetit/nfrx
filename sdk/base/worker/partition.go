...
func HandlePartitionedJob(
	...
	size := job.Size()
	if size <= 0 {
		return nil, http.StatusBadRequest, false, "empty"
	}
	if maxParallel <= 0 {
		maxParallel = 1
	}
	...
	if len(workers) == 0 {
		...
		if err != nil {
			return []byte(`{"error":"no_worker"}`), http.StatusNotFound, false, "no_worker"
		}
	...
	if len(workers) == 1 {
		...
		for start := 0; start < size; {
			...
			wk.RemoveJob(reqID)
			if obs != nil {
				obs.OnChunkResult(wk.ID(), model, time.Since(chunkStart), n, ok)
			}
			if !ok {
				if obs != nil {
					obs.OnJobResult(model, time.Since(jobStart), size, false)
				}
				return respBody, status, false, errMsg
			}
			if err := job.Append(respBody, start); err != nil {
				return []byte(`{"error":"aggregate_failed"}`), http.StatusBadGateway, false, "aggregate_failed"
			}
			start += n
		}
		if obs != nil {
			obs.OnJobResult(model, time.Since(jobStart), size, true)
		}
	...
	for i, w := range workers {
		n := 0
		if job != nil {
			n = job.DesiredChunkSize(w)
		}
		if n <= 0 {
			n = w.PreferredBatchSize()
		}
		if n <= 0 {
			n = size
		}
	...
	for i, w := range workers {
		...
		go func(first spi.WorkerRef, t task) {
			...
			for {
				...
				if n <= 0 {
					resCh <- result{start: t.start, body: nil, status: http.StatusBadRequest, err: "empty", ok: false}
					return
				}
				reqID := uuid.NewString()
				...
				if ok {
					resCh <- result{start: t.start, body: rb, ok: true, workerID: current.ID(), dur: time.Since(chunkStart), count: n}
					release(current)
					return
				}
				...
				if len(t.attempted) >= len(workers) {
					resCh <- result{start: t.start, body: rb, status: status, err: errMsg, ok: false, workerID: current.ID(), dur: time.Since(chunkStart), count: n}
					return
				}
				next, err := acquire(t.attempted)
				if err != nil {
					resCh <- result{start: t.start, body: rb, status: status, err: errMsg, ok: false}
					return
				}
				current = next
			}
	...
	for i := 0; i < len(tasks); i++ {
		r := <-resCh
		if obs != nil {
			obs.OnChunkResult(r.workerID, model, r.dur, r.count, r.ok)
		}
		if !r.ok {
			if obs != nil {
				obs.OnJobResult(model, time.Since(jobStart), size, false)
			}
			return r.body, r.status, false, r.err
		}
		if err := job.Append(r.body, r.start); err != nil {
			return []byte(`{"error":"aggregate_failed"}`), http.StatusBadGateway, false, "aggregate_failed"
		}
	}
	if obs != nil {
