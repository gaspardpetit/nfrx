...
func WSHandler(reg *Registry, metrics *MetricsRegistry, clientKey string, state spi.ServerState) http.HandlerFunc {
    return func(w http.ResponseWriter, r *http.Request) {
        ...
        if err != nil { logx.Log.Error().Err(err).Str("remote", r.RemoteAddr).Msg("ws accept"); return }
        ctx := r.Context()
        defer func() { _ = c.Close(websocket.StatusInternalError, "server error") }()

        _, data, err := c.Read(ctx)
        if err != nil { logx.Log.Error().Err(err).Str("remote", r.RemoteAddr).Msg("ws read register"); return }
        ...
        if err := json.Unmarshal(data, &rm); err != nil { logx.Log.Error().Err(err).Str("remote", r.RemoteAddr).Msg("ws decode register"); return }
        key := rm.ClientKey
        if clientKey == "" && key != "" { _ = c.Close(websocket.StatusPolicyViolation, "unauthorized"); return }
        if clientKey != "" && key != clientKey { _ = c.Close(websocket.StatusPolicyViolation, "unauthorized"); return }
        ...
        go func() {
            for msg := range wk.Send {
                b, err := json.Marshal(msg); if err != nil { continue }
                if err := c.Write(ctx, websocket.MessageText, b); err != nil { logx.Log.Error().Err(err).Str("worker_id", wk.ID).Str("worker_name", wk.Name).Msg("ws write"); return }
            }
        }()
        ...
        for {
            ...
            if err != nil {
                ...
                if errors.As(err, &ce) {
                    lvl := logx.Log.Info(); if ce.Code != websocket.StatusNormalClosure { lvl = logx.Log.Error() }
                    lvl.Str("worker_id", wk.ID).Str("worker_name", wk.Name).Str("reason", ce.Reason).Msg("disconnected")
                } else { logx.Log.Error().Err(err).Str("worker_id", wk.ID).Str("worker_name", wk.Name).Msg("disconnected") }
                return
            }
            var env struct{ Type string `json:"type"` }
            if err := json.Unmarshal(msg, &env); err != nil { logx.Log.Debug().Err(err).Str("worker_id", wk.ID).Str("worker_name", wk.Name).Msg("ws decode message"); continue }
            switch env.Type {
            case "heartbeat": reg.UpdateHeartbeat(wk.ID); metrics.RecordHeartbeat(wk.ID)
            case "status_update":
                var m ctrl.StatusUpdateMessage
                if err := json.Unmarshal(msg, &m); err == nil {
                    wk.mu.Lock(); wk.MaxConcurrency = m.MaxConcurrency; wk.EmbeddingBatchSize = m.EmbeddingBatchSize; if m.Models != nil { wk.Models = map[string]bool{}; for _, mm := range m.Models { wk.Models[mm] = true } } ; wk.mu.Unlock()
            ...
            case "job_chunk":
                ...
                if err := json.Unmarshal(msg, &m); err == nil { wk.mu.Lock(); ch, ok := wk.Jobs[m.JobID]; wk.mu.Unlock(); if ok { ch <- m } }
            case "job_result":
                var m ctrl.JobResultMessage
                if err := json.Unmarshal(msg, &m); err == nil { wk.mu.Lock(); ch, ok := wk.Jobs[m.JobID]; if ok { delete(wk.Jobs, m.JobID) } ; wk.mu.Unlock(); if ok { ch <- m; close(ch) } }
            case "job_error":
                var m ctrl.JobErrorMessage
                if err := json.Unmarshal(msg, &m); err == nil { wk.mu.Lock(); ch, ok := wk.Jobs[m.JobID]; if ok { delete(wk.Jobs, m.JobID) } ; wk.mu.Unlock(); if ok { ch <- m; close(ch) } }
            case "http_proxy_response_headers":
                var m ctrl.HTTPProxyResponseHeadersMessage
                if err := json.Unmarshal(msg, &m); err == nil { wk.mu.Lock(); ch, ok := wk.Jobs[m.RequestID]; wk.mu.Unlock(); if ok { ch <- m } }
            ...
            case "http_proxy_response_end":
                ...
                if err := json.Unmarshal(msg, &m); err == nil { wk.mu.Lock(); ch, ok := wk.Jobs[m.RequestID]; if ok { delete(wk.Jobs, m.RequestID) } ; wk.mu.Unlock(); if ok { ch <- m; close(ch) } }
            default:
