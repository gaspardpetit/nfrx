...
func probeBackend(ctx context.Context, cfg Config, ch chan<- ctrl.StatusUpdateMessage) error {
    ...
    if cfg.ProbeFunc != nil {
        ...
        SetLastError("")
        // Notify on connectivity flip or models change
...
func connectAndServe(ctx context.Context, cancelAll context.CancelFunc, cfg Config, statusUpdates <-chan ctrl.StatusUpdateMessage) (bool, error) {
    ...
    regMsg := ctrl.RegisterMessage{Type: "register", WorkerID: cfg.ClientID, WorkerName: cfg.ClientName, ClientKey: cfg.ClientKey, Models: GetState().Labels, MaxConcurrency: cfg.MaxConcurrency, AgentConfig: agentCfg}
