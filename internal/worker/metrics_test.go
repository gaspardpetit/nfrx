...
func TestMetricsServer(t *testing.T) {
	...
	data := string(body)
	if !strings.Contains(data, "infx_worker_connected_to_server 1") {
		t.Fatalf("missing connected_to_server gauge: %s", data)
	}
	if !strings.Contains(data, "infx_worker_connected_to_backend 1") {
		t.Fatalf("missing connected_to_backend gauge: %s", data)
	}
	if !strings.Contains(data, "infx_worker_jobs_started_total") {
		t.Fatalf("missing jobs_started_total counter: %s", data)
	}
	if !strings.Contains(data, "infx_worker_job_duration_seconds") {
