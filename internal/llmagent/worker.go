...
import (
	...
	"github.com/gaspardpetit/nfrx/internal/llmbackend"
...
func Run(ctx context.Context, cfg config.WorkerConfig) error {
	...
	client := llmbackend.New(strings.TrimSuffix(cfg.CompletionBaseURL, "/v1"))
...
func handleGenerate(ctx context.Context, client *llmbackend.Client, timeout time.Duration, sendCh chan []byte, jr ctrl.JobRequestMessage, cancels map[string]context.CancelFunc, mu *sync.Mutex, onDone func()) {
	...
	if req.Stream {
		...
		for line := range llmbackend.ReadLines(rc) {
			if idle != nil {
				if !idle.Stop() {
					<-idle.C
				}
