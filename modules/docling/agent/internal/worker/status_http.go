...
func StartStatusServer(ctx context.Context, addr, configFile string, drainTimeout time.Duration, shutdown func()) (string, error) {
    ...
    wrappedShutdown := func(){ SetState("terminating"); if shutdown!=nil { shutdown() } }
...
func defaultConfigPath(p string) string { if strings.TrimSpace(p)=="" { return "docling.yaml" } ; return p }
