...
func handleHTTPProxy(ctx context.Context, cfg aconfig.WorkerConfig, sendCh chan []byte, req ctrl.HTTPProxyRequestMessage, cancels map[string]context.CancelFunc, mu *sync.Mutex, onDone func()) {
    ...
    mu.Lock(); cancels[req.RequestID] = cancel; mu.Unlock()
    IncJobs()
    defer func(){ cancel(); mu.Lock(); delete(cancels, req.RequestID); mu.Unlock(); _ = DecJobs(); onDone() }()
    ...
    if err != nil { sendProxyError(reqCtx, req.RequestID, sendCh, err); return }
    for k, v := range req.Headers { if strings.EqualFold(k, "Authorization") { continue } ; httpReq.Header.Set(k, v) }
    if cfg.APIKey != "" { httpReq.Header.Set("Authorization", "Bearer "+cfg.APIKey) }
    ...
    for k, v := range resp.Header { hdrs[k] = strings.Join(v, ", ") }
    ...
    b, _ := json.Marshal(hmsg); sendMsg(reqCtx, sendCh, b)

    buf := make([]byte, 32*1024)
    for {
        n, err := resp.Body.Read(buf)
        if n>0 { cmsg := ctrl.HTTPProxyResponseChunkMessage{Type:"http_proxy_response_chunk", RequestID: req.RequestID, Data: append([]byte(nil), buf[:n]...)}; bb, _ := json.Marshal(cmsg); sendMsg(reqCtx, sendCh, bb) }
        if err != nil {
            if err == io.EOF { end := ctrl.HTTPProxyResponseEndMessage{Type:"http_proxy_response_end", RequestID: req.RequestID}; eb, _ := json.Marshal(end); sendMsg(reqCtx, sendCh, eb) } else { end := ctrl.HTTPProxyResponseEndMessage{Type:"http_proxy_response_end", RequestID: req.RequestID, Error: &ctrl.HTTPProxyError{Code:"upstream_error", Message: err.Error()}}; eb, _ := json.Marshal(end); sendMsg(reqCtx, sendCh, eb) }
            break
        }
    }
...
func sendProxyError(ctx context.Context, id string, sendCh chan []byte, err error) {
    ...
    eb, _ := json.Marshal(end); sendMsg(ctx, sendCh, eb)
