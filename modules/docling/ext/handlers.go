...
func proxyHandler(reg *baseworker.Registry, sched baseworker.Scheduler, mx *baseworker.MetricsRegistry, path string, timeout time.Duration) http.HandlerFunc {
    return func(w http.ResponseWriter, r *http.Request) {
        ...
        for {
            select {
            case <-ctx.Done():
                select { case wk.Send <- ctrl.HTTPProxyCancelMessage{Type:"http_proxy_cancel", RequestID: reqID}: default: }
                return
            case <-timeoutCh:
                hb := wk.LastHeartbeat
                ...
                errMsg = "timeout"
                select { case wk.Send <- ctrl.HTTPProxyCancelMessage{Type:"http_proxy_cancel", RequestID: reqID}: default: }
                if !headersSent { w.Header().Set("Content-Type","application/json"); w.WriteHeader(http.StatusGatewayTimeout); _, _ = w.Write([]byte(`{"error":"timeout"}`)) }
                return
            case msg, ok := <-ch:
                if !ok {
                    if !headersSent { w.Header().Set("Content-Type","application/json"); w.WriteHeader(http.StatusBadGateway); _, _ = w.Write([]byte(`{"error":"upstream_error"}`)) }
                    errMsg = "closed"; return
                }
                ...
                case ctrl.HTTPProxyResponseChunkMessage:
                    if len(m.Data) > 0 { _, _ = w.Write(m.Data); if flusher != nil { flusher.Flush() } }
                case ctrl.HTTPProxyResponseEndMessage:
                    if m.Error != nil { errMsg = m.Error.Code } else { success = true }
                    return
                }
            }
        }
...
func errMsgIf(cond bool, msg string) string { if cond { return msg }; return "" }
