...
func (embeddingObserver) OnChunkResult(workerID, model string, dur time.Duration, elements int, success bool) {
	...
	if elements > 0 {
		basemetrics.AddChunkSize("llm", "worker", "llm.embedding", model, workerID, "embeddings", uint64(elements))
	}
}
...
func (embeddingObserver) OnJobResult(model string, dur time.Duration, elements int, success bool) {
	...
	if elements > 0 {
		basemetrics.AddSize("llm", "worker", "llm.embedding", model, "embeddings", uint64(elements))
	}
}
