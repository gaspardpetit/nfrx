...
func (MCPAdapter) ValidateRequest(body []byte) (label string, id any, payload []byte, status int, errCode string, ok bool) {
	...
	if !ok {
		return "", nil, nil, http.StatusOK, mcpcommon.ErrSchema, false
	}
...
func (MCPAdapter) Open(ctx context.Context, rl *tunnel.Relay, sid, reqID, label, auth string) (bool, string, int) {
	...
	b, _ := json.Marshal(f)
	rl.Mu.Lock()
	err := rl.Conn.Write(ctx, 1, b)
	rl.Mu.Unlock()
	if err != nil {
		return false, mcpcommon.ErrProviderUnavailable, http.StatusServiceUnavailable
	}
	return true, "", 0
}
...
func (MCPAdapter) WaitOpen(ctx context.Context, ch <-chan []byte) (bool, string, int) {
	...
	case data := <-ch:
		...
		if f.T != "open.ok" {
			code := mcpcommon.ErrProviderUnavailable
			status := http.StatusServiceUnavailable
			if f.Code == mcpcommon.ErrUnauthorized {
				code = f.Code
...
func (MCPAdapter) WaitResponse(ctx context.Context, ch <-chan []byte, maxRespBytes int64) ([]byte, bool, string, int) {
	...
	case data := <-ch:
		var f mcp.Frame
		if json.Unmarshal(data, &f) != nil {
			return nil, false, mcpcommon.ErrProviderUnavailable, http.StatusServiceUnavailable
		}
		if int64(len(f.Payload)) > maxRespBytes {
			return nil, false, mcpcommon.ErrLimitExceeded, http.StatusOK
		}
