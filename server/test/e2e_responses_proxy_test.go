...
func TestE2EResponsesProxy(t *testing.T) {
	...
	ollama := httptest.NewServer(http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
		...
		case "/api/tags":
			...
			if _, err := w.Write([]byte(`{"models":[{"name":"gpt-4.1"}]}`)); err != nil {
		...
		case "/v1/responses":
			...
			fl := w.(http.Flusher)
			w.WriteHeader(200)
			if _, err := w.Write([]byte("event: response.created\ndata: {\"id\":\"resp_1\"}\n\n")); err != nil {
				t.Fatalf("write chunk1: %v", err)
			}
			fl.Flush()
			if _, err := w.Write([]byte("event: response.completed\ndata: {\"response\":{\"usage\":{\"input_tokens\":1,\"output_tokens\":2}}}\n\n")); err != nil {
	...
	go func() {
		probe := func(pctx context.Context) (wp.ProbeResult, error) {
			return wp.ProbeResult{Ready: true, Models: []string{"gpt-4.1"}, MaxConcurrency: 2}, nil
		}
	...
	reqBody := []byte(`{"model":"gpt-4.1","stream":true}`)
	req, _ := http.NewRequest(http.MethodPost, srv.URL+"/api/llm/v1/responses", bytes.NewReader(reqBody))
