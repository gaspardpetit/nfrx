...
func (c *ServerConfig) ApplyEnv() {
    if v := commoncfg.GetEnv("CONFIG_FILE", ""); v != "" { c.ConfigFile = v }
    if v := commoncfg.GetEnv("LOG_LEVEL", ""); v != "" { c.LogLevel = v }
    if v := commoncfg.GetEnv("PORT", ""); v != "" { if n, err := strconv.Atoi(v); err == nil { c.Port = n } }
    if v := commoncfg.GetEnv("METRICS_PORT", ""); v != "" {
        if strings.Contains(v, ":") { c.MetricsAddr = v } else { c.MetricsAddr = ":" + v }
    } else if c.MetricsAddr == "" {
        c.MetricsAddr = fmt.Sprintf(":%d", c.Port)
    }
    if v := commoncfg.GetEnv("API_KEY", ""); v != "" { c.APIKey = v }
    if v := commoncfg.GetEnv("CLIENT_KEY", ""); v != "" { c.ClientKey = v }
    if v := commoncfg.GetEnv("REDIS_ADDR", ""); v != "" { c.RedisAddr = v }
    if v := commoncfg.GetEnv("REQUEST_TIMEOUT", ""); v != "" { if f, err := strconv.ParseFloat(v, 64); err == nil { c.RequestTimeout = time.Duration(f * float64(time.Second)) } }
    if v := commoncfg.GetEnv("DRAIN_TIMEOUT", ""); v != "" { if d, err := time.ParseDuration(v); err == nil { c.DrainTimeout = d } }
    if v := commoncfg.GetEnv("ALLOWED_ORIGINS", ""); v != "" { c.AllowedOrigins = splitComma(v) }
    if v := commoncfg.GetEnv("PLUGINS", ""); v != "" { c.Plugins = splitComma(v) }
}
