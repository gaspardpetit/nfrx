...
import (
	...
	"github.com/gaspardpetit/nfrx-sdk/mcpwire"
...
type Relay struct {
	conn     *websocket.Conn
	mu       sync.Mutex
	pending  map[string]chan mcpwire.Frame
	inflight int
	lastSeen time.Time
...
func (r *Registry) WSHandler(clientKey string) http.HandlerFunc {
	return func(w http.ResponseWriter, req *http.Request) {
		...
		relay := &Relay{conn: c, pending: map[string]chan mcpwire.Frame{}, lastSeen: time.Now(), methods: map[string]int{}, sessions: map[string]sessionInfo{}, name: reg.ClientName}
...
func (r *Registry) pingLoop(ctx context.Context, clientID string, relay *Relay) {
	...
	for {
		...
		case <-ticker.C:
			...
			_ = relay.write(context.Background(), mcpwire.Frame{T: "ping"})
		case <-ctx.Done():
			return
		}
	}
...
func (rl *Relay) register(sid string) chan mcpwire.Frame {
	ch := make(chan mcpwire.Frame, 4)
	rl.mu.Lock()
	rl.pending[sid] = ch
	rl.mu.Unlock()
	return ch
...
func (rl *Relay) write(ctx context.Context, f mcpwire.Frame) error {
...
func (r *Registry) HTTPHandler() http.HandlerFunc {
	return func(w http.ResponseWriter, req *http.Request) {
		...
		defer cancel()
		if err := relay.write(ctx, mcpwire.Frame{T: "open", SID: sid, ReqID: reqID, Hint: env.Method, Auth: auth}); err != nil {
		...
		case <-ctx.Done():
			_ = relay.write(context.Background(), mcpwire.Frame{T: "close", SID: sid, Msg: "timeout"})
		...
		if err := relay.write(ctx, mcpwire.Frame{T: "rpc", SID: sid, Payload: raw}); err != nil {
			_ = relay.write(context.Background(), mcpwire.Frame{T: "close", SID: sid, Msg: "relay_write_failed"})
			logx.Log.Warn().Str("component", "server.http").Str("client_id", clientID).Str("req_id", reqID).Str("error_code", "MCP_PROVIDER_UNAVAILABLE").Msg("relay write failed")
			writeRPCError(w, env.ID, http.StatusServiceUnavailable, "MCP_PROVIDER_UNAVAILABLE", "relay write failed", reqID)
			return
		}
		var resp mcpwire.Frame
		select {
		case resp = <-ch:
			if len(resp.Payload) > int(r.maxRespBytes) {
				_ = relay.write(context.Background(), mcpwire.Frame{T: "close", SID: sid, Msg: "resp_too_large"})
		...
		_ = relay.write(context.Background(), mcpwire.Frame{T: "close", SID: sid, Msg: "done"})
